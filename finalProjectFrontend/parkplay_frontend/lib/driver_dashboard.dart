...
class _DriverDashboardState extends State<DriverDashboard> {
  ...
  void _loadAllLots() async {
    var lots = await _apiService.getAllParkingLots();
    setState(() => _allLots = lots);
  }

  String _getSpotDetails(int spotId) {
    for (var lot in _allLots) {
      for (var spot in lot.spots) {
        if (spot.id == spotId) {
          return "${lot.name} (Locul ${spot.spotNumber})"; // Ex: Iulius Mall (Locul A-5)
        }
      }
    }
    return "Loc ID: $spotId"; // Fallback dacă nu îl găsește
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(children: [
                        ...
                        SizedBox(width: 5),
                        Text("${_getSpotDetails(res.spotId)}"),
                      ]),
                      SizedBox(height: 4),
...
class _CreateReservationDialogState extends State<CreateReservationDialog> {
  ...
  void _reserve() async {
    ...
    if (_selectedSpot == null || _selectedDate == null || _startTime == null || _endTime == null) {
      setState(() {
        _errorMessage = "Te rog completează toate câmpurile!";
      });
      return;
    }
    ...
    if (startDateTime.isAfter(endDateTime) || startDateTime.isAtSameMomentAs(endDateTime)) {
      setState(() {
          _errorMessage = "Ora de sfârșit trebuie să fie după ora de start!";
        });
      return;
    }
    ...
    if (startDateTime.isBefore(DateTime.now())) {
      setState(() {
          _errorMessage = "Nu poti face rezervari în trecut!";
        });
      return;
    }
    ...
    try {
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(backgroundColor: Colors.green, content: Text("Rezervare reușită!"))
        );
      } else {
        setState(() {
          _errorMessage = "Locul este deja rezervat în acest interval!";
        });
      }
    } catch (e) {
      setState(() {
